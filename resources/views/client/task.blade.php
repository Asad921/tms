...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="{{ route('admin.dashboard') }}" class="brand-link">
      ...
      <span class="brand-text font-weight-light">SJ Client Dashboard</span>
    </a>
  ...
  <div class="content-wrapper">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('client.task') }}">Task</a></li>
               ...
               <div class="modal fade" id="addmenu" tabindex="-2" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"> --}}
                    <div class="modal-dialog" role="dialog">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('client.add_task') }}" method="POST" enctype="multipart/form-data">
    ...
    <table-section class="m-2">
      @if(!empty($projects))
@foreach ($projects as $project)
@if ($project->task->where('task_assigner', Auth::id())->count() > 0)    
...
<thead>
  ...
  <tbody>
    {{-- @php
      $user = $project->task->where('task_assigner', Auth::id())
    @endphp
    @dd($user[5]->title) --}}
    @foreach ($project->task->where('task_assigner', Auth::id())->sortByDesc('priority') as $task)
    <tr class="w-100">
        ...
        <div class="avatar d-flex">
          <div>
            ...
            <div class="detail ml-2 ">
              <a href="{{ route('client.task_details',$task->id) }}" class="text-dark font-weight-bold ">{{ $task->title ?? '' }}</a>     
              <div>
   ...
   <?php } ?>
        ...
        <td>
          ...
          <div class="btn-group align-middle mt-3" style="width: 10%">

            <a type="button" onclick="collapseComments({{ $task->id }})">
              <i class="fas fa-comments"></i>
              <span class="badge badge-danger navbar-badge " id="status_update{{ $task->id }}">{{ $task->comments->where('is_read', 0)->count() ?? 0 }}</span>
            </a>
          </div>
          ...
          <a class="dropdown dropdown-list-toggle text-dark">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                <div class="dropdown-list-content dropdown-list-icons">
                    <a href="{{ route('client.task_edit',$task->id) }}" class="dropdown-item dropdown-item-desc edit-btn"
                       data-id="1"><i
        ...
        <td class="w-100" colspan="12">
          ...
          @foreach ($task->comments as $comment) 
            <div class="row">
              ...
              <div class="col-md-3">
                <p class="text-gray">{{ $comment->created_at->diffForHumans() }}</p>
              </div>
              <div class="col-md-2">
                <a  type="button" class="text-dark"  href="{{ route('client.cc_delete',$comment->id) }}">
...
<script>
  function taskComment(task_id){
      ...
      function CommentStatus(id){
          ...
          var route = "{{ route('client.cc_update', ':id') }}";
      ...
      function project(id){
          var route = "{{ route('client.project_member', ':id') }}";
          route = route.replace(":id", id);
          $.ajax({
            type:'GET',
            url:route,
        ...
        function taskStatus(id){
          ...
          $.ajax({
            ...
            success:function(response){
              if(response.status == true){
                /* alert(response.message) */
                 window.location = "{{ route('client.task') }}"
              }
              else
                alert(response.message)
            }
